 ...
 * @since 01/03/2022
 *  * First version of the incremental propag. didnt work, tried V1 V3 and reccursiv'.
 */
public class PropRIV_VARandSet_IncrV2 extends Propagator<Variable> {
    ...
    public PropRIV_VARandSet_IncrV2(TriviumAlban triviumData) {
    ...
    private void newModif(int node, int profondeur)throws ContradictionException{
        shouldIPrint("newModif on node:"+node +" call="+profondeur);
        ...
        if(graph.getPotentialSuccessorsOf(node).size()==0 && !trivium.isNodeSink(node)) {//maybe be 'this' has remove previous nodes.
            //removeNode(node);
            //if(graph.getPotentialNodes().contains(node)) System.out.println(node+" really ?"+profondeur);
            return ;
        }
        if(graph.getPotentialPredecessorOf(node).size()==0 && !trivium.isNodeSource(node)) {//maybe be 'this' has remove succ nodes.
            //removeNode(node);
            //if(graph.getPotentialNodes().contains(node))  System.out.println(node+" really Pred?"+profondeur);
            return;
        }
        ...
        if(bool1 ||bool2 ||bool3 ||bool4)//if done something
        {
            for(int s:previousNeiboor){//graph.getPotentialSuccessorsOf(node)){
        ...
        else
        {
            ...
            if(doublingP.length==1) {
                newModif(doublingP[0], profondeur + 10);
            }
            if(doublingP.length==2) {
                newModif(doublingP[1], profondeur + 10);
            }

            if(profondeur<10) {
                for(int s:previousNeiboor)newModif(s, profondeur + 10);
            }
        }
    }
    ...
    private void filter() throws ContradictionException {
        if(computeRivUB()){
            if(outOfFirstPropag) System.out.println("in filter : computeRivUB filtered something after all others, search node="+trivium.m.getSolver().getNodeCount());
        }
        if(computeRivLB()){
            if(outOfFirstPropag) System.out.println("in filter : computeRivLB filtered something after all others, search node="+trivium.m.getSolver().getNodeCount());
        }
    }
    ...
    private void shouldIPrint(String s){
        //if(model.getSolver().getNodeCount()==50) System.out.println(s);
    }
