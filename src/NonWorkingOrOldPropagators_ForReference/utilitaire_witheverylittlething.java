...
public class utilitaire_witheverylittlething {
    ...
    public static PropRIV_alban propagRIV;
     ...
     */
    public static String getNodeName(int idx){

        String add="";//TO add RIV infos at the end if we have them.
        if(propagRIV!=null && propagRIV.RivLB!=null&& propagRIV.RivUB!=null){
            add = "_"+propagRIV.RivLB[idx]+"_"+propagRIV.RivUB[idx];
        }
        if(idx==tmp_idxSink) return "sink"+add;
        if(idx==tmp_idxSource) return "source"+add;

        if(idx<tmp_nbMaxNodePerRegistre){//registreA
            if(idx<=tmp_nbInnerProfondeur) return "A"+idx+add;
            return "x"+(idx-tmp_nbInnerProfondeur)+add;
        }
        if(idx<2*tmp_nbMaxNodePerRegistre){//registreB
            if(idx<=tmp_nbMaxNodePerRegistre+tmp_nbInnerProfondeur)
                return "B"+(idx-tmp_nbMaxNodePerRegistre)+add;
            return "iv"+((idx-tmp_nbInnerProfondeur)-tmp_nbMaxNodePerRegistre)+add;

        }
        if(idx<3*tmp_nbMaxNodePerRegistre){//registreC
            if(idx<=2*tmp_nbMaxNodePerRegistre+tmp_nbInnerProfondeur)
                return "C"+(idx-2*tmp_nbMaxNodePerRegistre)+add;
            return "z"+((idx-tmp_nbInnerProfondeur)-2*tmp_nbMaxNodePerRegistre)+add;
        }
 ...
 */
    ...
    public static void graphVizExport(TriviumAlban triv, PropRIV_alban propag) {
        propagRIV = propag;
        graphVizExport(triv);
    }
